...
const buttonVariants = cva(
  'inline-flex items-center justify-center rounded-full font-medium transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none disabled:cursor-not-allowed cursor-pointer',
  {
    variants: {
      variant: {
      ...
      size: {
        sm: 'h-8 px-3 py-1.5 text-sm gap-1.5 min-h-[44px] sm:min-h-0', // Touch-friendly on mobile, compact
        md: 'h-9 px-4 py-2 text-sm gap-2', // Compact default size
        lg: 'h-10 px-5 py-2.5 text-base font-semibold gap-2', // Larger but still compact
        icon: 'h-9 w-9 p-1.5 min-h-[44px] min-w-[44px]', // WCAG touch target, compact
      },
    },
    defaultVariants: {
      variant: 'primary',

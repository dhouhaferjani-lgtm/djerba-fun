...
<head>
    <meta charset="utf-8">
    <title>Vouchers - {{ $booking->booking_number }}</title>
    <style>
        @page {
            margin: 0;
        }
...
<body>
    @foreach($vouchers as $index => $voucherData)
        <div class="voucher {{ $index < count($vouchers) - 1 ? 'page-break' : '' }}">
            @php
                $participant = $voucherData['participant'];
            ...
            <div class="footer">
                ...
                <p>
                    ...
                    Voucher {{ $index + 1 }} of {{ count($vouchers) }}
                </p>
            </div>
        </div>
    @endforeach

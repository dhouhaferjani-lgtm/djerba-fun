function filamentGoogleGeocomplete({
  ...
  let geocompleteEl = isLocation ? statePath + '-fgm-address' : statePath,
  ...
  return {
    ...
    init: function (mapEl) {
      (console.log('geocomplete init'), (this.mapEl = mapEl), this.loadGMaps());
    },
    createAutocomplete: function () {
      ...
      let geocompleteOptions = { fields, strictBounds: !1, types };
      if (geoComplete) {
        ...
        let geoLocate = document.getElementById(statePath + '-geolocate');
        geoLocate &&
          ...
          geoLocate.addEventListener('click', (event) => {
            'geolocation' in navigator &&
              navigator.geolocation.getCurrentPosition((position) => {
                ...
                this.geocoder.geocode({ location: currentLocation }).then((response) => {
                  response.results[0] &&
    ...
    updateLatLng: async function (place) {
      Object.keys(latLngFields).length > 0 &&
        place.geometry &&
...
export { filamentGoogleGeocomplete as default };

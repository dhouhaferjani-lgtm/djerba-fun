function filamentGoogleMapsField({
  ...
  drawingModes = { marker: !0, circle: !0, rectangle: !0, polygon: !0, polyline: !0 },
  drawingField,
  geoJson,
  geoJsonField,
  geoJsonProperty,
  ...
  return {
    ...
    createMap: function () {
      if (
        ...
        (autocompleteReverse || Object.keys(reverseGeocodeFields).length > 0) &&
        ...
        (this.marker = new google.maps.Marker({ draggable, map: this.map })),
      ...
      if (
        (hasPlaceUpdatedUsing &&
          (this.placesService = new google.maps.places.PlacesService(this.map)),
        (this.placeFields = ['address_components', 'formatted_address', 'geometry', 'name']),
        this.placeFields.includes(placeField) || this.placeFields.push(placeField),
        hasPlaceUpdatedUsing && this.placeFields.push('photos'),
        autocomplete)
      ) {
        ...
        if (geoComplete) {
          window.addEventListener(
            ...
            function (e) {
              if (
                (e.key === 'U+000A' || e.key === 'Enter' || e.code === 'Enter') &&
          ...
          let geocompleteOptions = { fields: this.placeFields, strictBounds: !1, types },
            gAutocomplete = new google.maps.places.Autocomplete(geoComplete, geocompleteOptions);
          (gAutocomplete.setComponentRestrictions({ country: countries }),
            gAutocomplete.addListener('place_changed', () => {
              let place = gAutocomplete.getPlace();
              ...
              (place.geometry.viewport
                ...
                hasPlaceUpdatedUsing && placeUpdatedUsing(place));
            }));
        }
      }
      if (
        (layers &&
          (this.layers = layers.map((layerUrl) => {
            new google.maps.KmlLayer({ url: layerUrl, map: this.map }).addListener(
              'click',
              (kmlEvent) => {
        ...
        geolocateOnLoad && this.getLocation(),
        geolocate && 'geolocation' in navigator)
      ) {
      ...
      if (
        ...
        (this.drawingManager = new google.maps.drawing.DrawingManager({
          ...
          polylineOptions: { draggable: !0, editable: !1 },
          ...
          google.maps.event.addListener(this.drawingManager, 'overlaycomplete', (event) => {
            ((event.overlay.type = event.type),
              ...
              event.type != google.maps.drawing.OverlayType.MARKER &&
                (this.drawingManager.setDrawingMode(null), this.setSelection(event.overlay)),
      ...
      this.$watch('state', () => {
        ...
        (location.lat === markerLocation.lat() && location.lng === markerLocation.lng()) ||
          (this.updateAutocompleteFromLocation(location), this.updateMap(location));
      });
    },
      ...
      (this.geoJsonContains(event.latLng),
        ...
        hasPlaceUpdatedUsing &&
          ...
          this.placesService.getDetails(
            { placeId: event.placeId, fields: this.placeFields },
            (results, status) => {
    ...
    updateMap: function (position) {
      (this.marker.setPosition(position), this.map.panTo(position));
    },
    updateFromLocation: function (location) {
      (this.hasReverseGeocode() || this.hasReverseAutocomplete()) &&
        this.geocoder
          .geocode({ location })
          .then((response) => {
            (this.updateGeocodeFromAddressComponents(response.results[0].address_components),
              ...
              hasReverseGeocodeUsing && reverseGeocodeUsing(response));
          })
          .catch((error) => {
    ...
    updateGeocodeFromAddressComponents: function (address_components) {
      if (this.hasReverseGeocode()) {
        let replacements = this.getReplacements(address_components);
        for (let field in reverseGeocodeFields) {
          ...
          ((replaced = replaced.trim()), setStateUsing(field, replaced));
        }
      }
    },
    updateGeocodeFromLocation: function (location) {
      this.hasReverseGeocode() &&
        this.geocoder
          .geocode({ location })
    ...
    updateAutocompleteFromFormattedAddress: function (address) {
      this.hasReverseAutocomplete() && setStateUsing(autocomplete, address);
    },
    updateAutocompleteFromLocation: function (location) {
      this.hasReverseAutocomplete() &&
        this.geocoder
          .geocode({ location })
          .then((response) => {
            response.results[0] &&
    ...
    getLocation: function () {
      navigator.geolocation.getCurrentPosition((position) => {
        ((this.markerLocation = { lat: position.coords.latitude, lng: position.coords.longitude }),
          this.setCoordinates(this.markerLocation),
          this.updateFromLocation(this.markerLocation),
          this.map.panTo(this.markerLocation));
      });
    ...
    getReplacements: function (address_components) {
      ...
      return (
        ...
        debug && console.log(replacements),
        replacements
      );
    },
        ...
        case google.maps.drawing.OverlayType.RECTANGLE:
          let b = overlay.getBounds(),
            p = [
              ...
              b.getNorthEast(),
              { lng: b.getSouthWest().lng(), lat: b.getNorthEast().lat() },
            ];
          return geometryOnly
        ...
        for (let a2 = 0; a2 < $a.length; a2++)
          !Array.isArray($a[a2]) ||
            transform(
              $a[a2],
              parent
                ? $a[a2].length == 2 && !Array.isArray($a[a2][0]) && !Array.isArray($a[a2][1])
                  ? parent
      ...
      switch (overlay.type) {
        case google.maps.drawing.OverlayType.POLYLINE:
          (google.maps.event.addListener(overlay.getPath(), 'set_at', () => {
            overlay.drag ||
              (overlay.feature.setGeometry(this.calculateGeometry(overlay, !0)),
              this.drawingModified());
          }),
    ...
    setSelection: function (shape) {
      (this.clearSelection(),
        ...
        this.overlays.forEach(function (item) {
          shape && item.id == shape.id
            ? (item.getEditable()
                ? shape.setOptions({ strokeColor: '#00ff00', strokeOpacity: '0.5' })
                : shape.setOptions({ strokeColor: '#ff0000', strokeOpacity: '0.8' }),
    ...
    clearSelection: function () {
      ((this.selectedShape = null),
        this.overlays.forEach(function (item) {
          (item.setEditable(!1), item.setOptions({ strokeColor: '#00ff00', strokeOpacity: '0.5' }));
        }));
    },
      ...
      if (geoJson && geoJsonField) {
        ...
        this.geoJsonDataLayer.forEach((feature) => {
          if (feature.getGeometry().getType() === 'Polygon') {
            var poly = new google.maps.Polygon({ path: feature.getGeometry().getAt(0).getArray() });
            ...
            feature
              ...
              .forEach(function (item, i) {
                let coords = item.getAt(0).getArray(),
                  poly2 = new google.maps.Polygon({ paths: coords });
                google.maps.geometry.poly.containsLocation(latLng, poly2) &&
                  (geoJsonProperty
                    ? features.push(feature.getProperty(geoJsonProperty))
                    : dataLayer.add(feature));
              });
        ...
        let fieldContent;
        geoJsonProperty
          ? ((fieldContent = JSON.stringify(features)), setStateUsing(geoJsonField, fieldContent))
          : dataLayer.toGeoJson((gj) => {
              ((fieldContent = JSON.stringify(gj)), setStateUsing(geoJsonField, fieldContent));
            });
      }
    },
  };
